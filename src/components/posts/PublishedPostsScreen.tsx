...
import { PostCard } from "./published/PostCard";
import { PageHeader } from "./published/PageHeader";
import { FiltersToolbar } from "./published/FiltersToolbar";
import { EmptyState } from "./published/EmptyState";
import { PostsGrid } from "./published/PostsGrid";
...
export default function PublishedPostsPage() {
  ...
  const [watcherPostIds, setWatcherPostIds] = useState<Set<string>>(new Set());
  const [togglingWatcherId, setTogglingWatcherId] = useState<string | null>(null);

  const loadWatcherIds = async () => {
    try {
      const enabled = await listEnabledWatcherPosts("facebook");
      setWatcherPostIds(new Set(enabled.map((p) => p.post_id)));
    } catch {
      /* optional billing gate */
    }
  };

  const handleToggleAiWatcher = async (postId: string, pageId: string, enabled: boolean) => {
    setTogglingWatcherId(postId);
    try {
      if (enabled) {
        await enableAiWatcher({ post_id: postId, platform: "facebook", page_id: pageId });
      } else {
        await disableAiWatcher({ post_id: postId, platform: "facebook" });
      }
      await loadWatcherIds();
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : "AI Watcher action failed");
    } finally {
              ...
              return (
                <div key={pageId} className="space-y-4">
                  ...
                  <PostsGrid
                    ...
                    watcherPostIds={watcherPostIds}
                    onToggleAiWatcher={handleToggleAiWatcher}
                    togglingWatcherId={togglingWatcherId}
                  />
                </div>
              );
            })
