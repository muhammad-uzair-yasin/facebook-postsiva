...
import { createDraft, getDraft } from "@/lib/hooks/facebook/drafts/api";
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (draft.media_id) setSelectedMediaId(draft.media_id);
        if (draft.image_url) setPreviewUrls([draft.image_url]);
      } catch (err) {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await createDraft({
        ...
        image_url: previewUrls[0] ?? undefined,
        source_type: "manual",
      });
      setShowSuccess(true);
    } catch (err) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-4 sm:gap-6 md:gap-8 lg:gap-10 items-start">
        ...
        <div className="space-y-4 sm:space-y-6 md:space-y-8">
          ...
          <div className="flex flex-col md:flex-row gap-4 sm:gap-6 items-end">
            <section className="w-full md:w-auto space-y-3 sm:space-y-4 flex flex-col sm:flex-row gap-3">
              <Button
                type="button"
                variant="outline"
                className="w-full h-12 sm:h-14 rounded-xl sm:rounded-2xl text-sm sm:text-base font-black"
                onClick={() => void handleSaveDraft()}
                disabled={savingDraft || !selectedPageId}
              >
